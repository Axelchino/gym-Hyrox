...
export function useSaveHyroxProgress(userId: string | null | undefined) {
  ...
  return async (patch: Partial<Pick<HyroxProgress, 'done' | 'times' | 'benchmarks' | 'stations' | 'pillarDayMap' | 'recoveryChoices' | 'tier' | 'raceDate' | 'planId' | 'targetTotalSeconds' | 'doublesTargetSeconds' | 'planStartDate'>>) => {
