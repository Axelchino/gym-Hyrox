...
function mapProgress(row: any): HyroxProgress {
  return {
    ...
    doublesTargetSeconds: row.doubles_target_seconds ?? 3761,
...
type ProgressPatch = Partial<Pick<HyroxProgress, 'done' | 'times' | 'benchmarks' | 'stations' | 'pillarDayMap' | 'recoveryChoices' | 'tier' | 'raceDate' | 'planId' | 'targetTotalSeconds' | 'doublesTargetSeconds' | 'planStartDate'>>;
...
export async function upsertMyProgress(patch: ProgressPatch): Promise<void> {
  ...
  if (patch.doublesTargetSeconds !== undefined) row.doubles_target_seconds = patch.doublesTargetSeconds;
